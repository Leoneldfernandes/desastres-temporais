...
def main() -> None:
    ...
    summary = load(args.data / "atlas-summary.json")
    geometry = load(args.data / "geo" / "municipios-br.geojson")
    ...
    uf_files = sorted((args.data / "events").glob("*.json"))
    state_geo_files = sorted((args.data / "geo" / "uf").glob("*.json"))
    ...
    for path in uf_files:
        ...
        events_by_uf[path.stem] = len(payload["rows"])
